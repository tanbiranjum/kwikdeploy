...
public record EnvCreate : IRequest<Result<int>>
{
    [FromRoute]
    public int ProjectId { get; init; }

    [FromBody]
    public EnvCreateBody Body { get; init; } = null!;
}

public record EnvCreateBody
{
...
public class EnvCreateHandler : IRequestHandler<EnvCreate, Result<int>>
{
    private readonly IApplicationDbContext _context;

    public EnvCreateHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<Result<int>> Handle(EnvCreate request, CancellationToken cancellationToken)
    {
